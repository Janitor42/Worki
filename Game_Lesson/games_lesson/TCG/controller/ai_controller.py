...
class AIController:
    ...
    def find_card_from_table(self):
        cards = self.find_group_card(atr='table')
        card_sorted = [x for x in cards if x.get_state_attack()]
        return card_sorted
    ...
    def play_card_from_hand(self):
        cards = self.find_cards_from_hand()
        if not cards:
            return False
        target = cards[0]
        ...
        self.player_model.remove_and_send_on_table(target=target.get_attr('_name'))
        self.player_model.set_energy(energy=int(target.get_attr('_cost')))
        return True

    def play_card_from_table(self, enemy_player_model):
        cards = self.find_card_from_table()
        if not cards:
            return False
        target = cards[0]
