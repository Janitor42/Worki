...
def check_ammunition():
    ...
    if value_reload > 0:
        action = int(input('Заряжаем (1 begin) или взводим и стреляем? (0)'))
        return action
    if value_reload == 0:
        action = int(input('Барабан пустой, для зарядки нажмите (1 begin) '))

def add_ammunition(check_ammunition):
    global reload, value_reload
    if action == 1 and value_reload < 6:
        reload = int(input('Для зарядки патрона нажмите (1 begin) '))
        if reload == 1:
            value_reload += 1
...
def preparing():
    global prepar
    if value_reload>0:
        prepar=int(input('для взведения курка (1 begin) если не хотим (0)'))


def shoot():
    global fire
    if prepar==1:
        fire=int(input('Можем стрельнуть (1 begin) или пропустить (0) '))
        return fire
