...
class Balls:
    ...
    def __init__(self, x, y):
        ...
        self.x = x  # x
        self.y = y  # y
        self.__speed_x = rd_not_zero(-3, 3)

        self.__speed_y = define_y(self.__speed_x)

        # self.name = sp.add('pacman', self.x, self.y, 'player3')
        self.name = sp.add_text('O', self.x, self.y, font_name='Arial',text_color=(255,255,0),bold=True)
        sp.set_width_proportionally(self.name, 16)

        Balls.all_balls.append(self)
    ...
    def move(self):
        self._check_window()
        sp.move(self.name, self.__speed_x, 0)
        self.check_collide_x()
        sp.move(self.name, 0, self.__speed_y)
