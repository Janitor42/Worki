...
# tasks.txt = [asyncio.create_task(task_coro(i)) for i in range(10)]
...
# done, pending = await asyncio.wait(tasks.txt)
...
# done, pending = await asyncio.wait(tasks.txt, timeout=3)
...
#     tasks.txt = [asyncio.create_task(task_coro(i)) for i in range(10)]    # создание нескольких задач
#     done, pending = await asyncio.wait(tasks.txt)# ожидание завершения выполнения всех задач
...
# async def main():
#     tasks.txt = [task_coro(i) for i in range(3)]
#
#     try:
#         # Объединяем все задачи в одну с помощью gather() и ждём с таймаутом
#         results = await asyncio.wait_for(asyncio.gather(*tasks.txt), timeout=0.5)
...
#     tasks.txt = []  # Создание пустого списка для хранения задач
#     for i in range(10):
#         tasks.txt.append(task_coro(i))  # Добавление корутины в список (но пока не запуск!)
#
#
#
#     done, pending = await asyncio.wait(tasks.txt, timeout=2.1)    # Ожидание выполнения задач с таймаутом 2.1 секунды
