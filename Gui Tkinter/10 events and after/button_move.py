...
def collide():
    global x_dot,y_dot
    if x_bt+10 in [i for i in range(x_dot-10,x_dot+size+10)] and  y_bt+10 in [i for i in range(y_dot-10,y_dot+size+10)]:
...
def move_to_me(event):
    # print(type(event))- тип
    # print(event.time) - время вызова эвента в формате python
    # print(event.x) - позиция x у обьекта вызова
    # print(event.x_root) - позиция x касательно экрана
